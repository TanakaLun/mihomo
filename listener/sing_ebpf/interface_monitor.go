...
// has no process-wide sing-box NetworkManager singleton.
func (i *Inbound) ebpfNetworkMonitor() tun.NetworkUpdateMonitor {
	return nil
}

// ebpfInterfaceMonitor returns nil so the default-interface monitor is created
// locally from the shared interface finder.
func (i *Inbound) ebpfInterfaceMonitor() tun.DefaultInterfaceMonitor {
	return nil
}
...
func (i *Inbound) startTCInterfaceMonitor() error {
	networkMonitor := i.ebpfNetworkMonitor()
	networkOwned := false
	...
	defaultInterfaceMonitor := i.ebpfInterfaceMonitor()
	defaultInterfaceOwned := false
	if defaultInterfaceMonitor == nil {
		var err error
		defaultInterfaceMonitor, err = tun.NewDefaultInterfaceMonitor(networkMonitor, log.SingLogger, tun.DefaultInterfaceMonitorOptions{
...
func (i *Inbound) updateTCInterfaces(ctx context.Context) (outcome tcUpdateOutcome) {
	...
	sharedDataPlane := i.sharedRewriteDataPlane()
	if sharedDataPlane != nil {
		...
		if err = sharedDataPlane.Reconcile(sharedInterfaces, hostAddresses); err != nil {
			...
			outcome.sharedRewrite = i.sharedRewriteRetryOutcome()
		} else {
	...
	if err = i.udpReplySockets.reset(); err != nil {
		i.interfaceWarnings.reconcile.warn(i.logWarn, "reset TC eBPF UDP reply sockets: ", err)
		outcome.general = tcSharedRewriteRecoverable
	}
	...
	if !slices.Equal(previousAttachments, attachments) {
		log.Debugln("eBPF TC attachments updated: attachments=[%s]", strings.Join(attachments, ", "))
	}
	if outcome.general == tcSharedRewriteUnknown {
		outcome.general = tcSharedRewriteSettled
	}
