...
func tcLinkFraming(link netlink.Link) (ECommon.TCLinkFraming, error) {
	if link == nil || link.Attrs() == nil {
		return ECommon.TCLinkFramingUnsupported, E.New("invalid TC eBPF interface")
	}
	attributes := link.Attrs()
	framing := ECommon.ClassifyTCLinkFraming(attributes.EncapType, len(attributes.HardwareAddr))
	if framing == ECommon.TCLinkFramingUnsupported {
